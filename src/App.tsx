...
export default function App() {
  ...
  useEffect(() => {
    fetch('/api/config')
      .then(res => res.json())
      .then(data => {
        setServerOnline(data.hasApiKey || !!localStorage.getItem('gemini_api_key'));
...
<br>
পরামর্শ শুরু করতে, দয়া করে:<br>
১. **বাম দিকের ক্লিনিক্যাল ইনটেক ফর্মে আপনার প্রাথমিক তথ্য দিন** (অথবা দ্রুত মূল্যায়নের জন্য একটি প্রিসেট নির্বাচন করুন)।</br>
২. চিকিৎসা প্রসঙ্গ সিঙ্ক করতে **"সক্রিয় প্রোফাইল সেট করুন"**-এ ক্লিক করুন।<br>
৩. চিকিৎসা বা পরামর্শ তৈরি করার জন্য আপনার অন্য কোন সমস্যার কথা লিখুন বা প্রশ্ন করুন। <br>`,
          ...
          <div className="flex items-center gap-4">
            ...
            {serverOnline === false && (
              <span 
                ...
                className="text-[10px] bg-red-950/40 hover:bg-red-900/50 border border-red-900/50 text-red-200 px-2 py-0.5 rounded font-medium flex items-center gap-1 cursor-pointer transition-colors"
                title="এপিআই কী সেট করুন"
              >
            ...
            <button
              onClick={() => setShowSettings(true)}
              className="px-2.5 py-1 bg-slate-805 hover:bg-slate-800 text-slate-300 hover:text-blue-400 rounded-md border border-slate-600 transition-all cursor-pointer flex items-center justify-center gap-1 text-[11px] font-bold"
              title="সেটিংস"
            >
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full overflow-hidden border border-slate-200">
            <div className="p-4 border-b border-slate-200 flex justify-between items-center">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                <Settings className="w-4 h-4 text-slate-500" /> অ্যাপ সেটিংস
              </h3>
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-200 flex justify-end">
               <button onClick={handleSaveSettings} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded text-xs font-bold transition-all shadow-xs">সেভ করুন</button>
            </div>
          </div>
        </div>
      )}
